...
import {User, locationSchema, settingSchema, Message, ClinicPatient} from '../models';
...
class AdminController {
    ...
    static async dashboard () {
        return new Promise(async(resolve,reject)=>{
            try {
                const [totalProvider,totalLocations] = await Promise.all([
                    DbOperations.count(User, {userType:1,is_deleted:false}),
                    DbOperations.count(locationSchema, {isActive:true}),
                ])
                return resolve({totalProvider:totalProvider,totalLocations:totalLocations});
            } catch (err) {
                return reject(err);
            }
        })
